...
def detect_edges(frame, showDetec=True):  # Créé une nouvelle image avec les bordure des objets bleu

    hsv = cv2.cvtColor(frame,
                       cv2.COLOR_BGR2HSV)  # On transforme le format BRG en HSV pour éviter les différentes teintes de bleu due à la luminosité
    ...
    mask = cv2.inRange(hsv, lower_blue,
                       upper_blue)  # On ne garde que les couleurs entre le bleu clair et le bleu foncé (à noter que open CV utilise une gamme de couleur entre 0 et 180)
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    mask_cleaned = cv2.morphologyEx(cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel), cv2.MORPH_CLOSE, kernel)
    # img_clean = cv2.addWeighted(cv2.bitwise_and(frame, frame, mask=mask_cleaned), 1, np.zeros_like(frame), 0, 0)
    edges = cv2.Canny(mask_cleaned, 200, 400)  # On ne garde que les contours
    if (showDetec):
        cv2.imshow("mask", edges)  # On affiche la nouvelle image
...
def detect_lane(frame, showDetec=True):  # Fonction qui résume les précédentes

    edges = detect_edges(frame, showDetec)
    # cropped_edges = region_of_interest(edges)
...
def test_vidéo():
    ...
    while videoStream.isOpened():
        ...
        lane_lines = detect_lane(frame, show)
        steering_angle = compute_steering_angle(frame, lane_lines)
        heading_image = display_heading_line(frame, steering_angle)

        print("Angle : %i", steering_angle)

        cv2.imshow("Detection d'objets", frame)
...
def test_video_picam():
    try:
        i = 16
        f_count = 0
        total_time = 0
        show = False
        curr_steering_angle = stabilized_steering_angle = 90
        while True:
            start_time = time.perf_counter()

            # Capture de la frame
            acq = picam2.capture_array()
            frame = cv2.rotate(cv2.cvtColor(acq, cv2.COLOR_RGB2BGR), cv2.ROTATE_180)

            # Détection de voies
            edges, lane_lines = detect_lane(frame, show)
            if len(lane_lines)>0:
                new_steering_angle = compute_steering_angle(frame, lane_lines)
            ...
            heading_image = display_heading_line(frame, stabilized_steering_angle, (255,255,255))
            ...
            total_time += p_time
            f_count += 1
            avg_time = total_time / f_count
            fps = 1 / avg_time if avg_time > 0 else 0

            # Affichage des résultats
            print(f"t = {p_time:.4f}s ; FPS = {fps:.2f} ; Angle = {stabilized_steering_angle:.1f}°")
            cv2.imshow("Detection d'objets", heading_image)
...
picam2 = Picamera2()
config = picam2.create_still_configuration({"size": (640, 360)})
